...
export const Dashboard: React.FC = () => {
  ...
  const handleDownloadPDF = async (id: string, fullName: string) => {
    ...
    try {
      const fullResume = await ApiService.getResume(id);
      const blob = await ApiService.exportPdf(fullResume);
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      ...
      console.error('Error fetching/downloading PDF:', error);
      alert(`Failed to load or generate PDF: ${error.message || error}`);
    } finally {
      setDownloadingId(null);
      setDownloadType(null);
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImportError(null);
    try {
      const parsedResume = await ApiService.importResume(file, selectedModel);
